 ...
 * @param max 最大值
 */
const randomNumberByRange = (min = 0, max = 1000): number => Math.floor(Math.random() * (min - max) + max);
...
export const randomBoolean = () => randomNumber(10) % 2 === 0;

export const randomNumber = (range = 1000) => Math.floor(Math.random() * range);

export const randomString = (prefix = '', length = 4) => prefix + Math.random().toString(36).slice(-length);
